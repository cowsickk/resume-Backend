  ...
  return text.split("\n\n").filter(b => b.trim()).map(block => {
    ...
    const inner = `<div style="${timeline ? "flex:1;" : "margin-bottom:10px;"}">
      ...
      ${rest.length ? `<ul style="list-style:none;padding:0;margin-top:3px;">${rest.map(l =>
        `<li style="font-size:11.5px;color:${bc};padding-left:0;margin-bottom:3px;line-height:1.55;">
          ${stripLeadingListSymbol(l)}
        </li>`
      ).join("")}</ul>` : ""}
    </div>`;
...
function firstFilledValue(obj, keys) {
...
function findValueByKeyPattern(obj, pattern, depth = 0) {
  if (!obj || typeof obj !== "object" || depth > 5) return "";
  for (const [key, value] of Object.entries(obj)) {
    if (pattern.test(key) && v(value)) return value;
    if (value && typeof value === "object") {
      const nested = findValueByKeyPattern(value, pattern, depth + 1);
      if (v(nested)) return nested;
    }
  }
  return "";
}

function findAdditionalInfo(body) {
  const keys = [
    "additionalInfo",
  ...
  const direct = firstFilledValue(body, keys);
  if (v(direct)) return direct;

  const nestedKeys = ["resume", "resumeData", "formData", "data", "state", "values"];
  for (const nestedKey of nestedKeys) {
    const nested = body?.[nestedKey];
    const nestedValue = firstFilledValue(nested, keys);
    if (v(nestedValue)) return nestedValue;
  }

  return findValueByKeyPattern(
    body,
    /additional|course\s*work|coursework|extra\s*info|other\s*info/i
  );
}
...
app.post("/generate-resume", async (req, res) => {
  try {
    ...
    const additionalInfo = v(findAdditionalInfo({
      ...req.body,
      additionalInfo: additionalInfoBody,
      additionalInformation,
      additional_information,

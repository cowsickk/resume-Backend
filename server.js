...
async function callAI(prompt) {
  ...
  if (!data.choices || !data.choices.length) throw new Error("Invalid AI response: " + JSON.stringify(data));
...
// Extracted into a shared helper so both T_EXECUTIVE and
// T_ACADEMIC call the EXACT same HTML, eliminating the
// copy-paste divergence that caused the missing section.
function renderAdditionalInfo(additionalInfo, languages, fontSize = "11.5px") {
  // Check the RAW values — not v() which always returns a string
  const hasInfo = additionalInfo && String(additionalInfo).trim().length > 0;
  const hasLang = languages && String(languages).trim().length > 0;
  if (!hasInfo && !hasLang) return "";
  return `<div style="font-size:${fontSize};line-height:1.7;color:#333;">
    ${hasInfo ? `<div><strong>Course Work:</strong> ${String(additionalInfo).trim()}</div>` : ""}
    ${hasLang ? `<div style="margin-top:${hasInfo ? "3px" : "0"}"><strong>Languages Known:</strong> ${String(languages).trim()}</div>` : ""}
  </div>`;
}
...
<div class="rr">
  ...
  ${(additionalInfo && String(additionalInfo).trim()) || (languages && String(languages).trim()) ? `<div class="sec"><div class="lbl"><span>Additional</span></div><div class="vbar"></div><div class="body">${renderAdditionalInfo(additionalInfo, languages)}</div></div>` : ""}
</div>
${fitScript}</body></html>`;

    /* ════════════════════════════════════════════════════
       TEMPLATE 2 — EXECUTIVE
       FIX: removed overflow:hidden from html/body so the
       Additional Information section is never clipped.
       The Puppeteer fit-script handles single-page constraint.
       FIX: replaced inline <p> tag (which inherits a global
       CSS rule that may not apply inside template strings)
       with renderAdditionalInfo() which uses inline styles.
    ════════════════════════════════════════════════════ */
...
<div class="rr">
  ...
  <div class="bd">
    ${v(summary) ? `<div class="st">Professional Summary</div><div style="font-size:11.5px;line-height:1.65;color:#333;text-align:justify;">${summary}</div>` : ""}
    ...
    ${v(leadership) ? `<div class="st">Leadership &amp; Event Conductions</div><div style="font-size:11.5px;line-height:1.65;color:#333;">${leadership}</div>` : ""}
    ${(additionalInfo && String(additionalInfo).trim()) || (languages && String(languages).trim()) ? `<div class="st">Additional Information</div>${renderAdditionalInfo(additionalInfo, languages)}` : ""}
  </div>
</div>
${fitScript}</body></html>`;
...
<div class="rr">
  ...
  ${v(summary) ? `<div class="st">Professional Summary</div><div style="font-size:11.5px;line-height:1.65;color:#222;text-align:justify;">${summary}</div>` : ""}
  <div class="st">Technical Skills</div>${renderSkills(skillCategories, skills, "rows")}
...
<div class="rp">
  ...
  <div class="cn">
    ...
    ${v(summary) ? `<div class="sec"><div class="tl"><div class="dot"></div><div class="line"></div></div><div class="sb"><div class="st">Summary</div><div style="font-size:11px;line-height:1.65;color:#333;">${summary}</div></div></div>` : ""}
    ...
    ${v(leadership) ? `<div class="sec"><div class="tl"><div class="dot"></div><div class="line"></div></div><div class="sb"><div class="st">Leadership</div><div style="font-size:11px;line-height:1.65;color:#333;">${leadership}</div></div></div>` : ""}
    ${(additionalInfo && String(additionalInfo).trim()) || (languages && String(languages).trim()) ? `<div class="sec"><div class="tl"><div class="dot"></div></div><div class="sb"><div class="st">Additional Information</div>${renderAdditionalInfo(additionalInfo, languages, "11px")}</div></div>` : ""}
  </div>
</div>
${fitScript}</body></html>`;
    ...
    /* ════════════════════════════════════════════════════
       ...
       FIX: Additional Information moved BEFORE the grid2
       block so it no longer gets buried inside a CSS grid
       cell that may collapse to zero height when the sibling
       cell is taller. It now sits as its own full-width row.
       FIX: replaced <p> with inline-style <div> so font-size
       is guaranteed (global `p` selector may not cascade in
       Puppeteer's sandboxed HTML context).
    ════════════════════════════════════════════════════ */
...
<div class="rr">
  ...
  ${v(summary) ? `${banner("Summary")}<div style="font-size:11.5px;line-height:1.65;color:#222;text-align:justify;">${summary}</div>` : ""}
  ...
  ${v(leadership) ? `${banner("Leadership &amp; Event Conductions")}<div style="font-size:11.5px;line-height:1.65;color:#222;">${leadership}</div>` : ""}
  ${(additionalInfo && String(additionalInfo).trim()) || (languages && String(languages).trim()) ? `${banner("Additional Information")}${renderAdditionalInfo(additionalInfo, languages)}` : ""}
</div>
${fitScript}</body></html>`;

    /* ════════════════════════════════════════════════════
       TEMPLATE MAP & PUPPETEER PDF
    ════════════════════════════════════════════════════ */
    const htmlMap = {
